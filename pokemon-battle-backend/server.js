...
app.get('/api/pokemon', async (req, res) => {
  try {
    ...
    const response = await axios.get('http://apiexall.com:3001/api/pokemon');
...
app.get('/api/pic_poke', async (req, res) => {
  try {
    ...
    const response = await axios.get(`http://apiexall.com:3001/api/pic_poke?pok_name=${pokemonName}`);
...
app.post('/api/user_id', async (req, res) => {
  try {
    ...
    const sql = "SELECT * FROM al_user_id WHERE user_name = $1";
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    const checkUserSql = "SELECT * FROM al_user_id WHERE user_name = $1";
    ...
    const insertSql = "INSERT INTO al_user_id (user_name, pass) VALUES ($1, $2)";
...
app.get('/api/user_wl', async (req, res) => {
  try {
    ...
    const sql = "SELECT * FROM al_user_wl WHERE user_id = $1";
...
app.post('/api/save_game_result', async (req, res) => {
  try {
    ...
    const updateWLQuery = `
    INSERT INTO al_user_wl (user_id, win, lose, draw)
    VALUES ($1, $2, $3, $4)
    ON CONFLICT (user_id)
    DO UPDATE SET
    win = al_user_wl.win + $2,
    lose = al_user_wl.lose + $3,
    draw = al_user_wl.draw + $4`;
...
app.post('/api/update_exp_and_level', async (req, res) => {
  ...
  try {
    ...
    const userQuery = 'SELECT experience_points, level FROM al_user_id WHERE user_id = $1';
